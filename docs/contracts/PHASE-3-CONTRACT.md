...
| O-28 ▸ **DISCHARGED 2026-08-31 by D3.5** — every finding ruled, and closed by the director's read of `docs/contracts/PHASE-3-HISTORY-REVIEW.md`, same day. **The repository going public remains a separate explicit word, not yet given** | Git-history review before publication | **D3.5** |
...
## Questions raised at the launch build — Q33 and Q34

Raised 31 August 2026, after D3.11–D3.16 were built and before any irreversible
act. **Neither is ruled.** Q34 **blocks the tag**: a ruling that has not reached
this document is not binding, and the act it governs is already wired to fire.

### Q33 — An environment error is reported as a defect in this tool

Found by CI on `container.yml`'s first run and recorded as **C-52**. A bind
mount the container user cannot write produced:

```
INTERNAL ERROR [PermissionError] [Errno 13] Permission denied: '/work/run'
This is a defect in prevalence-kit, not a problem with your data.
```

**Both halves of that sentence are false here.** It is not a defect in
prevalence-kit, and it is not a problem with the data — it is the environment,
and the operator is sent hunting a bug that does not exist. That is the precise
harm `TW-4`'s own text warns about: *"whoever debugs it will start hunting a bug
that is not there."* The guard in `cli.py` treats every non-`Refusal` exception
as an internal defect, which was right when every reachable exception was one.

**This touches `cli.py`, which Phase 2 closed**, and §3's closing note reserves
that to the director — hence a question rather than a change.

| | Option | Consequence |
|---|---|---|
| **A** | **Distinguish environment errors from internal defects** in the guard: `PermissionError`, `IsADirectoryError`, `NotADirectoryError`, `OSError` on a path the operator supplied get a message naming the path, the likely cause and the fix; everything else keeps the internal-defect message **unchanged, for the case it was written for**. Both controls | The operator is sent to the right place. The internal-defect path is untouched, so the message that is correct for a real defect stays correct. Costs a narrow, testable widening of a closed file |
...
check red, per rule 21.

### Q34 — GHCR is a third publish channel, and no ruling names it

**Found by the reviewer, and it is wired to fire.** `container.yml`'s `publish`
job runs `if: startsWith(github.ref, 'refs/tags/v')` and pushes to
`ghcr.io/${{ github.repository }}` with an attestation. **D-43 ruled two
channels: a GitHub release and PyPI, with TestPyPI rehearsed first.** A
container registry is a third irreversible publish — an image pushed to GHCR is
public, immutable by digest, and cannot be unpublished — and it entered the
repository as a consequence of D3.12 rather than as a ruled channel.

`docs/SOP.md` already tells operators `docker pull ghcr.io/...`, so the
documentation and the workflow both assume a channel the record never
authorised. **This is the shape D-43 exists to prevent**, arriving one level
down.

The director's stated intent is that the container ship on GHCR, so this is a
question about *recording and rehearsing* it, not about whether to do it.

| | Option | Consequence |
|---|---|---|
| **A** | **Amend D-43 to three channels** — GitHub release, PyPI, GHCR — each with its rehearsal target named: TestPyPI for PyPI, and for GHCR **a `-rc` tag pushed to the same registry path**, which exercises the real registry, the real credentials and the real attestation while producing an artifact nobody is asked to depend on. D3.8 names all three; the SOP's `docker pull` line is then authorised | The channel is ruled rather than inherited, and the rehearsal exercises the full path — Template 6's requirement that a rehearsal not skip the risky steps. A published `-rc` image is permanent, which is stated rather than glossed |
| B | Three channels, GHCR rehearsed to a **separate** registry path (e.g. `…/prevalence-kit-rc`) | Keeps the real path clean of candidates. Costs fidelity: the rehearsal no longer proves the path the real push will take, which is the one thing a rehearsal is for |
| C | Drop GHCR: remove the publish job and the SOP's pull line | Two channels as ruled, and the container becomes build-it-yourself — against the director's stated intent and against the reason the container exists |

**Recommendation: A.** Note what it costs, so the ruling is made with it in view:
a release candidate pushed to the real path is **itself irreversible**, and the
record will carry a published `1.0.0rc1` image forever. That is the honest price
of rehearsing the actual path, and Template 6 says failed candidates stay in the
record anyway.
...
**CLOSED — 1 September 2026.** Written at the close, not after it.

**v1.0.0 is released.** Tag `v1.0.0` at `3b09267`; published to PyPI, to
`ghcr.io/mohdsaifhussain/prevalence-kit:1.0.0`, and as a GitHub release with the
wheel, the sdist and a CycloneDX SBOM attached. **The exit criterion is met**:
the published artifacts were verified as an outsider would verify them,
including negative controls that failed.

### The exit checklist, evidenced

| # | Result |
|---|---|
| G1 | TW-4 clear for the first time since it was written — five tripwires, none fired, exit 0 |
| G2 | `witness.yml` green on the re-pinned actions; every fixture regenerated byte-identically |
| G3 | The coverage artifact reproduces from its recorded procedure; the plot's curve is the artifact's numbers |
| G4 | Read by eye. Clopper-Pearson at or above nominal at all four points; Wilson below at the two commoner rates and above at the two rarer. **The reading was corrected at the stop — C-51** — because the first version claimed a gap larger than the Monte Carlo error it had defined five lines above |
| G5 | README read by eye: the O-21 block grouped as one fact, `svy` credited and machine-asserted, limits whole |
| G6 | The status sentence is true at the release commit, and machine-checked in both public files |
| G7 | Both directions proved: a perturbed sentence fails, a deleted sentence fails |
| G8 | `PHASE-3-HISTORY-REVIEW.md`, every finding ruled, closed by the director's read |
| G9 | Fresh `pip install prevalence-kit` in a clean environment; the SOP's commands reproduce the documented run exactly |
| G10 | **Five controls, each exit code captured directly.** Wheel and image verify against this repository; both refuse against a wrong repository; a wheel with one byte appended refuses |
| G11 | Re-run against the **published** artifacts after the release, not the working tree |
| G12 | Attestations verified with `gh attestation verify`; SBOM attached to the release |
| G13 | The ROOST pull request, submitted under the director's rules |
| G14 | Local and CI green, stated apart |
| G15 | Selftest 13/13, `check_claims` reconciled |
| G16 | T-1 closed every open correction, each naming a commit **verified by content** rather than by a message mentioning its number |
| G17 | `Phase 3 of 4 complete` in both public files |

### Defects found by running it, not by inspection

Numbered, because they are the phase's most valuable output.

1. **C-50** — *"both target Node 20"* was **false for `checkout` on the day it
   was written**, and stood in five sites across four documents. Found by the
   re-read the director ordered before acting on O-19. **Three hand-carried
   lists were each wrong**; only a search survived.
2. **C-51** — the coverage reading claimed Wilson fell below nominal *by more
   than the Monte Carlo error*, with that error defined five lines above and the
   arithmetic contradicting it. Rule 10's shape. **The reviewer's correction was
   also wrong**, on the upper side; the text that shipped came from performing
   all eight comparisons.
3. **C-52** — the documented `docker run` command **fails on Linux**. Caught by
   CI on `container.yml`'s first run; the identical commands had passed on
   Windows minutes earlier. **R3.12 earned its place the day it was written**:
   had the workflow only *built* the image, this would have shipped in v1.0's
   documentation.
4. **The rehearsal's own controls could not fail.** Piped into `tail`, so `$?`
   reported *tail's* status: both negatives printed the right error and reported
   success. Found and fixed before the release; every control since captures its
   exit code directly.
5. **Two rehearsal candidates failed** — a non-existent SBOM flag, then an
   unregistered trusted publisher. Both kept in `PHASE-3-REHEARSAL.md`.
6. **The `pypi` environment did not exist**, which would have failed the release
   job before it reached the index and looked like a publish failure.
7. **The registry reference was uppercase.** Verified locally rather than
   asserted: Docker rejects it, while the GitHub URL is case-insensitive.

### Deviations

1. **The phase was amended mid-flight**, after the review stop, by the
   director's launch-readiness direction: six deliverables **D3.11–D3.16**, and
   **Q26–Q34** ruled after the contract was approved. Recorded as an amendment
   in §3a rather than absorbed.
2. **Three charter §5.1 and §5.6 obligations had never been built** and were
   found by comparing this repository against its own siblings: no `LICENSE`
   while two artifacts claimed MIT (**F-A**), no dependency lock (**F-B**), and
   neither the overclaim scanner nor badge-truth tests. All three now exist.
3. **The suite roughly doubled in wall-clock time** and was profiled rather than
   assumed. Two causes were mine — a test helper copying tool caches and a
   1,000-file sealed store, and `check_figures` deriving the test count twice
   per call. Both fixed; no assertion weakened.

### Obligations

| # | Status |
|---|---|
| O-19 | **Discharged by D3.1**, premise re-read first — C-50 |
| O-21 | **Discharged by D3.4** — the rare-event facts grouped as one fact |
| O-28 | **Discharged by D3.5**, closed by the director's read |
| O-10 | **Discharged** — the `svy` credit is machine-asserted, absence a failure |
| O-14 | **To the NEXT queue by name** (D-49). A keyless structural audit mode: real, wanted, and not built |
| O-15 | **Unmet by design** (D-25, D-49). A ledger schema version is added only if an old run and an API-created run ever need different advice. That has not happened |
| Q-2 | Permanently **`noted`**. No test the builder writes can close *the suite is the builder's* |

### Honest limits

Charter §8 carried forward **unchanged**, and now stated in three public places:
the README, the release notes, and the tool's own report output. One addition
this phase measured rather than inherited: **`verify` recomputes through the
same estimator that produced the number** — it proves the record is intact and
self-consistent, and does not independently confirm the estimator.
