...
| O-13 ▸ **DISCHARGED 2026-08-30 by D2.10** | Measure the `svy` Wilson divergence at small *n*. **D-18's unmeasured claim is confirmed in direction and now has a size**, with every axis stated. The boundaries turned out to matter more than the magnitude: **zero-width at `k = n`, and no interval at all at `k = 0`** — which is the most common honest result in rare-event work | D-18 |
