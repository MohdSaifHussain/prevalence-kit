...
| **S-1.13** ▸ **ROW ADDED 2026-08-30** | **The two design rules for stratification** — strata are mutually exclusive and covering, and each is sampled independently. Anchors **Q12** and **Q14 / D-40** | **Statistics Canada**, *Power from Data!*, §3.2.2 *Probability sampling* | `https://www150.statcan.gc.ca/n1/edu/power-pouvoir/ch13/prob/5214899-eng.htm`, **date modified 2021-09-02**; re-fetched **2026-08-30, HTTP 200**. **An official Statistics Canada publication and an *educational* one** — not its methodology manual (*Survey Methods and Practices*, 12-587-X), which has not been read. The row says which, because the register described it as *"official methodology"* for a day and that is a stronger claim than the artifact supports. **C-43** | **2026-11-30** |
...
| **S-1.13** | **full** | **Statistics Canada**, *Power from Data!* §3.2.2, read 2026-08-29 and re-fetched 2026-08-30. Supplies the two design rules S-1.2 and S-1.3 were standing in for. **They are one sentence, not two quotations**: *"the population is divided into homogeneous, mutually exclusive groups called strata, and then independent samples are selected from each stratum."* The register carried them as two fragments, which reads as two independent statements of support. **C-43** |
...
| S-5.4a *(new, 2026-08-31)* | **Release-path actions, SHA-pinned** — D3.15 | `actions/upload-artifact` **v7.0.1** (`043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`); `actions/download-artifact` **v8.0.1** (`3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`); `pypa/gh-action-pypi-publish` **v1.14.2** (`dc37677b2e1c63e2034f94d8a5b11f265b73ba33`); `docker/login-action` **v4.6.0** (`dbcb813823bdd20940b903addbd779551569679f`); `docker/build-push-action` **v7.3.0** (`53b7df96c91f9c12dcc8a07bcb9ccacbed38856a`) | Every SHA resolved from the tag through the GitHub API on **2026-08-31**, dereferencing annotated tags to their commit — not copied from documentation. `actions/attest-build-provenance` is **S-5.2a** and is pinned to the version recorded there. **These run only on a tag or a manual dispatch**, never on a push | **every phase close** (TW-4 reads `gate.yml`; these are not in it, and that gap is stated in TW-4's own row) |
| S-5.4b *(new, 2026-08-31)* | **Build and SBOM tooling** — D3.15 | `build` **1.6.0**; `cyclonedx-bom` **7.3.1**, emitting CycloneDX JSON | Versions read live from PyPI on **2026-08-31**. Build-time only: neither is a runtime dependency and neither appears in `constraints.txt`, which locks what an operator installs | **at the release** |
| S-5.4c *(new, 2026-08-31)* | **Container base image, pinned by digest** — D3.12 / D-56 | `docker.io/library/python:3.14-slim` @ `sha256:cae66f2ef0ec51a9891263eeee7f987dacf0a9879e8aa9353d5606e0530619a5` | Resolved **2026-08-31** with `docker buildx imagetools inspect`. **By digest, not tag, for S-2.1a's reason**: a tag moves, a digest does not. Built and the shipped example run inside it before this row was written | **at the release** |
...
| S-8.5 | Fetch the coverage-demonstration corpus (S-7.1) | `curl -sSL https://huggingface.co/datasets/google/civil_comments/resolve/main/data/<file>` for the four parquet files the dataset ships | **2026-08-31**: four files, **422,061,071 bytes** total, per-file SHA-256 digests and row counts recorded in `demo/coverage_results.json`; rows total **1,999,514**, matching S-7.1 exactly. Executed **after** the pre-registration commit `ce25ce1`, which is the ordering D3.3 rests on | **at the release** |
